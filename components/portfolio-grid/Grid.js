...
import noContent from "../../public/assets/animations/no-content.json";
    ...
    <>
      {isLoading ? (
        <Lottie
          ...
          loop={true}
          className="absolute top-1/2 left-1/2 lg:-translate-x-0 -translate-x-1/2 -translate-y-1/2 h-60 w-60"
        />
      ) : (
        <>
          {images[0] && images[0]?.file ? (
            <>
              {images?.length > 0 && (
            ...
            <div className=" text-white absolute top-1/2 left-1/2 lg:-translate-x-0 -translate-x-1/2 -translate-y-1/2 ">
              <Lottie
                animationData={noContent}
                loop={true}
                className=" lg:h-60 lg:w-60 md:h-44 md:w-44 h-32 w-32"
              />
